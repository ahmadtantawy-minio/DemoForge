# DemoForge Enhancement Execution Plan — Multi-Phase Claude Code Instructions

## How to use this document

Each phase is a self-contained unit of work. Complete all tasks in a phase before moving to the next. Each phase ends with unit tests and E2E validation. Work in git worktrees:

```bash
git worktree add ../demoforge-phase-N feature/phase-N
cd ../demoforge-phase-N
```

Before starting ANY phase, read these files to understand conventions:
- `backend/app/models/demo.py` — DemoDefinition, DemoNode, DemoAnnotation models
- `backend/app/engine/compose_generator.py` — edge resolution patterns
- `frontend/src/types/index.ts` — all TypeScript types, ConnectionType union
- `frontend/src/stores/diagramStore.ts` — React Flow state management
...
- `frontend/src/components/canvas/DiagramCanvas.tsx` — node/edge type registration
- `demo-templates/*.yaml` — template structure, `_template:` metadata block

---

## Phase 1: Template Cleanup & Gallery Tiers (1-2 days)
...
class TemplateMetadata(BaseModel):
    name: str
    tier: str = "essentials"        # NEW: "essentials" | "advanced"
    category: str = "general"       # NEW: "lakehouse" | "infrastructure" | "ai" | "simulation"
...
```typescript
test('medallion experience is read-only', async ({ page }) => {
  // Load Medallion Experience
  // Verify can't drag nodes
  // Verify can't delete nodes
  // Verify palette is hidden
  // Verify annotations are visible
  // Verify Deploy button still works
});
...
| Phase | Scope | Effort | PoC scenarios covered |
|-------|-------|--------|----------------------|
| 1 | Template cleanup + gallery tiers | 1-2 days | UX foundation |
| 2 | SQL Playbook panel | 3-4 days | Enables 5 scenarios |
| 3 | Time travel + tiering templates | 0.5 day | Scenarios 5, 6 |
| 4 | Multi-table data gen + Customer 360 | 4-5 days | Scenario 1 |
| 5 | Medallion Experience | 1 day | Scenario 3 |
| 6 | Nested JSON + semi-structured | 3 days | Scenario 2 |
| 7 | CDC + OCR (if timeline permits) | 2 weeks | Scenarios 8, 9 |

After Phase 5, you cover 6/9 PoC scenarios. Scenario 7 (Generative Search) is already covered by the RAG pipeline template.
